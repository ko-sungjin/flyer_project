    ...
    if template_id == "1":  # 기존 1번 템플릿
        ...
        for i, item in enumerate(items[:5]):
            ...
            if (i + 1) % 5 == 0:
                x = margin
                y += cell_height + 10
    elif template_id == "2":  # 새로운 2번 템플릿: 이미지 왼쪽, 텍스트 오른쪽
        cell_width, cell_height = 280, 150  # 2열로 배치
        margin = 20
        x, y = margin, 70
        for i, item in enumerate(items[:4]):  # 4개 품목 표시 (2x2 그리드)
            # 이미지 왼쪽
            if item["ProcessedImagePath"]:
                try:
                    item_img = Image.open(item["ProcessedImagePath"])
                    item_img.thumbnail((100, 100))
                    img.paste(item_img, (x, y + 10), item_img if item_img.mode == "RGBA" else None)
                except:
                    draw.text((x, y + 10), "[이미지 없음]", font=smaller_font, fill="black")
            # 텍스트 오른쪽
            text_x = x + 110
            draw.text((text_x, y + 10), item["Name"][:20], font=small_font, fill="black")
            draw.text((text_x, y + 30), f"₩{item['Price']:,.0f} (각)", font=smaller_font, fill="red")
            if item.get("AdditionalPrice"):
                draw.text((text_x, y + 50), f"10g당 {item['AdditionalPrice']}원", font=smaller_font, fill="black")
            # 다음 셀로 이동
            x += cell_width + 10
            if (i + 1) % 2 == 0:  # 2열로 배치
                x = margin
...
def render_flyer(template_id, items, output_path, title, footer_text):
    ...
    c.setFillColorRGB(1, 1, 1)
    c.setFont("NotoSansKR", 20)
    c.drawCentredString(width / 2, height - 10 * mm, title)

    if template_id == "1":
        cell_width, cell_height = 40 * mm, 50 * mm
        x, y = margin, height - 25 * mm
        c.setFont("NanumGothic", 12)
        for i, item in enumerate(items[:5]):
            if item["ProcessedImagePath"]:
                c.drawImage(item["ProcessedImagePath"], x, y - 30 * mm, 30 * mm, 30 * mm)
            c.drawString(x, y - 35 * mm, item["Name"][:20])
            c.setFillColorRGB(1, 0, 0)
            c.drawString(x, y - 45 * mm, f"₩{item['Price']:,.0f} (각)")
            c.setFillColorRGB(0, 0, 0)
            if item.get("AdditionalPrice"):
                c.drawString(x, y - 55 * mm, f"10g당 {item['AdditionalPrice']}원")
            x += cell_width + 5 * mm
            if (i + 1) % 5 == 0:
                x = margin
                y -= cell_height + 5 * mm
